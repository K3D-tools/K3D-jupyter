...
module.exports = function (THREE) {
    ...
    function MeshLineMaterial(parameters, K3D) {
        const vertexShaderSource = [
            ...
            '',
            '    // a NaN separator on either side ends the run; folding it onto the current',
            '    // point takes the one-sided branch below instead of writing NaN into',
            '    // gl_Position, which drops the quad and with it a two-point run entirely',
            '    if( prevP != prevP ) prevP = currentP;',
            '    if( nextP != nextP ) nextP = currentP;',
            '',
