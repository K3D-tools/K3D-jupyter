...
float textures, does not cover every object (`volume_slice` is not drawn, `mip` stays
outside the light simulation, only the first `volume` is path traced), and its API and
output may still change; `simple` and `advanced` are the stable choices. See
